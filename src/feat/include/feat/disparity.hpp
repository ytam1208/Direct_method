#ifndef __DISPARITY__
#define __DISPARITY__
...
public:
    ...
    void calculate_disparity_map(cv::Mat& left, cv::Mat& right){
        ...
        // cv::normalize(this->left_disparity, this->show_disparity, 0, 255, cv::NORM_MINMAX, CV_8U);
    ...
    void calculate_disparity2depth_map(cv::Mat& left, cv::Mat& disparity){
        pointcloud.clear();
        this->depth = cv::Mat::zeros(left.rows, left.cols, CV_32FC1);
        float depth = 0.0f;
        float bad_point = std::numeric_limits<float>::quiet_NaN();
        float Min_depth = 0.0f;
        float Max_depth = 9999.0f;
        for(int v = 0; v < disparity.rows; v++)
            for(int u = 0; u < disparity.cols; u++){
                ...
                point[1] = y * depth;
                point[2] = depth;
                this->depth.at<float>(v,u) = depth;
                pointcloud.push_back(point);
                // std::cout << point[2] << ", ";
            }
    }
    void Display(cv::Mat& left, cv::Mat& right, bool Use_filter){
        // cv::normalize(this->depth, this->depth, 0, 255, cv::NORM_MINMAX, CV_8U);
        try{
            cv::imshow("L", left);
            cv::imshow("R", right);
    ...
    Depth_Map():
    focal_length(718.856f), base_line(0.54f), fx(718.856f), fy(718.856f), cx(607.192f), cy(185.215f){
        initalize();
    }
    ~Depth_Map(){}
};
